...
impl User {
    ...
    /// Get the current card for a contact on the primary device.
    pub async fn get_contact_card(&self, name_or_id: &str) -> E2eResult<Option<ContactCard>> {
        let primary = self
            .primary_device()
            .ok_or_else(|| E2eError::user("No primary device"))?;

        let device = primary.read().await;
        device.get_contact_card(name_or_id).await
    }
    ...
    pub async fn exchange_with(&self, other: &User) -> E2eResult<()> {
        ...
        // Both sessions must exist before either side completes; the CLI
        // persists the session ephemeral generated by `start`.
